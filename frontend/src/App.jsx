...
function App() {
  ...
  const maxStock = Math.max(...products.map(product => product.stockLevel), 1)
          ...
          <div className="space-y-7">
            ...
            <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="text-slate-500 mt-1">Manage stock, pricing, and product signals</p>
              </div>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full min-w-[760px] text-sm">
                  ...
                  <tbody>
                    {loading ? (
                      <tr><td colSpan="6" className="p-10 text-center text-slate-500"><Loader2 className="animate-spin inline"/></td></tr>
                    ) : products.length === 0 ? (
                      ...
                      <tr key={product.id} className="border-t border-slate-100 hover:bg-slate-50">
                        <td className="px-5 py-4"><div className="font-medium">{product.name}</div><div className="text-[11px] text-slate-500 font-mono">{product.sku}</div></td>
                        <td className="px-3 py-4 text-slate-600">{product.category}</td>
                        <td className="px-3 py-4 text-right"><span className={product.stockLevel < product.reorderThreshold ? 'text-amber-600 font-semibold' : 'text-slate-700'}>{product.stockLevel}</span><span className="text-[10px] text-slate-500"> / {product.reorderThreshold}</span></td>
                        <td className="px-3 py-4 text-right font-mono">${Number(product.currentPrice).toFixed(2)}</td>
                        <td className="px-3 py-4"><span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-[11px] font-semibold text-slate-600">{product.status?.replaceAll('_', ' ')}</span></td>
                        <td className="px-5 py-4"><div className="flex justify-end gap-2"><button onClick={() => sale(product.id)} disabled={working === `sale-${product.id}`} className="inline-flex items-center gap-1 rounded-lg bg-teal-500 px-3 py-2 text-xs font-bold text-white hover:bg-teal-600 disabled:opacity-50"><ShoppingCart size={14}/> Sale</button><button onClick={() => suggest(product.id, 'pricing')} disabled={working === `pricing-${product.id}`} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50">Price</button><button onClick={() => suggest(product.id, 'reorder')} disabled={working === `reorder-${product.id}`} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50">Reorder</button></div></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="space-y-7">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6">
                ...
                {products.length === 0 ? (
                  <div className="h-64 flex items-center justify-center text-sm text-slate-500">No product data available yet.</div>
                ) : (
                  <div className="space-y-4">
                    {products.map(product => (
                      <div key={product.id} className="grid grid-cols-[minmax(110px,1fr)_3fr_52px] items-center gap-3 text-xs">
                        <span className="truncate text-slate-700" title={product.name}>{product.name}</span>
                        <div className="relative h-7 rounded-md bg-slate-100">
                          <div className={`h-7 rounded-md ${product.stockLevel < product.reorderThreshold ? 'bg-amber-400' : 'bg-teal-500'}`} style={{ width: `${Math.max((product.stockLevel / maxStock) * 100, 3)}%` }} title={`${product.stockLevel} in stock`}/>
                          <div className="absolute top-0 h-7 border-l-2 border-slate-700/60" style={{ left: `${Math.min((product.reorderThreshold / maxStock) * 100, 100)}%` }} title={`Reorder threshold: ${product.reorderThreshold}`}/>
                        </div>
                        <span className="text-right font-semibold text-slate-700">{product.stockLevel}</span>
                      </div>
                    ))}
                    <div className="flex items-center gap-4 pt-2 text-[11px] text-slate-500"><span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-teal-500"/> Healthy stock</span><span className="inline-flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-amber-400"/> Below threshold</span><span className="inline-flex items-center gap-1"><span className="h-3 border-l-2 border-slate-700/60"/> Reorder point</span></div>
                  </div>
                )}
              </div>
